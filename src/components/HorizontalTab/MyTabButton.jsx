...
const MyTabButton = ({ children, selected, value, onChange,
    type, selectedIsMatchPath, rounded = 'lg', fullwidth = false }) => {
    ...
    return (
        <>
            <button data-test={`btn-tab-setting-${value}`} onClick={() => onChange && onChange(value)} className={`flex items-center gap-2 whitespace-nowrap ${makeStyle}`}>
                {children}
            </button>
        </>
    );
