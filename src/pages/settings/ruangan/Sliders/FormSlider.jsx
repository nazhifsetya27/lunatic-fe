...
function FormSlider() {
  ...
  const onSubmit = handleSubmit(
    handleError(currentSlider.id ? updateRoom : createRoom, control),
    checkErrorYup
  )

  useEffect(() => {
    if (currentSlider.id) {
      showRoom(currentSlider.id).then((data) => {
        setTitle(data.name)
            ...
            <footer className="sticky bottom-0 flex items-center justify-end gap-4 border-t border-gray-light/200 bg-white px-4 py-4">
              ...
              >
                <MyButton
                  ...
                  onClick={() => deleteRoom(currentSlider.id)}
                  variant="text"
                  size="md"
                >
                  <p
              ...
              {deleted_at ? (
                <MyButton
                  onClick={() => restoreRoom(currentSlider.id)}
                  type="reset"
                  color="primary"
                  variant="outlined"
                  size="md"
                >
                  <Refresh className="size-5" stroke="currentColor" />
