...
function FormSlider() {
  ...
  const onSubmit = handleSubmit(
    handleError(currentSlider.id ? updateFloor : createFloor, control),
    checkErrorYup
  )

  useEffect(() => {
    if (currentSlider.id) {
      showFloor(currentSlider.id).then((data) => {
        setTitle(data.name)
            ...
            <footer className="sticky bottom-0 flex items-center justify-end gap-4 border-t border-gray-light/200 bg-white px-4 py-4">
              ...
              >
                <MyButton
                  ...
                  onClick={() => deleteFloor(currentSlider.id)}
                  variant="text"
                  size="md"
                >
                  <p
              ...
              {deleted_at ? (
                <MyButton
                  onClick={() => restoreFloor(currentSlider.id)}
                  type="reset"
                  color="primary"
                  variant="outlined"
                  size="md"
